...
public class LimitedLengthRun : Modifier
{
    ...
    {
        if (state.stageManager.playerPosZ >= distance)
        {
            state.stageManager.playerController.Life = 0;
        }
    }
...
public class SingleLifeRun : Modifier
{
    public override void OnRunTick(GameState state)
    {
        if (state.stageManager.playerController.Life > 1)
            state.stageManager.playerController.Life = 1;
    }
